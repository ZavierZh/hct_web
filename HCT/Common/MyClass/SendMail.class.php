...
class SendMail {
    ...
    * @access protected
    */
    protected $_sendServer="smtp.exmail.qq.com";
    /**
    * @var int 邮件传输代理服务器端口
    * @access protected
    */
...
//     protected $callback_error;
    
    public function __construct(){
    	if ($this->_userName != ""){
    		$this->_userName=base64_encode($this->_userName);
    	}
    	if ($this->_password != ""){
    ...
    */
    ...
    public function setUser($username,$password,$fromname=""){
    	if(!$username || !$password){
    		$this->_errorMessage = "没有用户名或密码";
    		return false;
    	}
    	$this->_userName = base64_encode($username);
    	$this->_password = base64_encode($password);
    	$this->_from = $username;
    	$this->_sendServer = 'smtp.exmail.qq.com';
    	$this->_port = 465;
    	$this->_isSecurity = true;
    	if($fromname) $this->_from_name=$fromname;
    	return true;
    }
    ...
    */
     ...
     * @param function $func
     */
//     public function setCallbackError($func){
//     	if(function_exists('func')){
//     		$this->_errorMessage = "设置回调函数出错";
//     		return false;
//     	}
//     	$this->callback_error = $func;
//     	return true;
//     }
    /**
    * 返回mail命令
    ...
    */
    protected function getCommand() {
    	if(!$this->_userName ){
    		$this->_errorMessage = "用户信息不完整";
    		goto error;
    	}
    	if(!$this->_password ){
    		$this->_errorMessage = "密码信息不完整";
    		goto error;
    	}
    	if(!$this->_from ){
    		$this->_errorMessage = "没有发送人";
    		goto error;
    	}
    	if(!$this->_sendServer){
    		$this->_errorMessage = "邮箱服务器信息不完整";
    		goto error;
    	}
    	if(!$this->_port ){
    		$this->_errorMessage = "端口信息不完整";
    		goto error;
    	}
    	if(!$this->_to ){
    		$this->_errorMessage = "没有收件人";
    		goto error;
    	}
    	if(!$this->_subject){
    		$this->_errorMessage = "没有主题";
    		goto error;
    	}
    	if(!$this->_body){
    		$this->_errorMessage = "没有正文";
    		goto error;
    	}
...
//         		$this->callback_error($this->_errorMessage);
        	return false;
    }
    /**
    * 发送命令
